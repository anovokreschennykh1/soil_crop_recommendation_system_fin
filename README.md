...
(2) Синтетически датасет содержащий 1189 наблюдения с климатическими и почвенными характеристиками synthetic_soil_climate_crop_dataset.csv
...
- Novokreschennykh_final_project_clean.ipynb - тетрадка в которой содержится код по импорту и дообучению классификатора фотографий + реализация классификатора по рекомендации посевных культур
- synthetic_soil_climate_crop_dataset.csv - синтетический датасет для обучения классификатора CatBoost
- crop_images - директория с фотографиями растений для вывода в рекомендательной ленте пользователю. Ее необходимо поместить в ту же директорию, что и исполняющим файл
- [Soil-Classification-Dataset
](https://github.com/Phantom-fs/Soil-Classification-Dataset/tree/main) можно загрузить командой в начале тетрадки
